...
public class LevelGenerator {
    ...
    public LevelGenerator(int groundLength, int courtWidth, int courtHeight) {
        ...
        // Create the Stairs array 
        stairs = new LinkedList<Stairs>();
        i = 1;
        for (; i <= 6; i++) {
        	for(int j = 1 ; j <= i ; j ++) {
        		stairs.add(new Stairs(courtWidth, courtHeight, (groundLength - 29 + i * 2) * GroundTile.SIZE , j)); // 樓梯從倒數第20格開始
        	}	
        }
    ...
    public LinkedList<Stairs> getStairs(){
    	LinkedList<Stairs>St = new LinkedList<Stairs>();
